...
import io.github.dd1android.launcher.xserver.util.Callback;
...
public class XClient extends ConnectedClient implements XResourceManager.OnResourceLifecycleListener {
    ...
    private final LinkedHashMap<Window, EventListener> eventListeners = new LinkedHashMap<>();
    ...
    public void sendEvent(Event event) {
        ...
        io.github.dd1android.launcher.xconnector.XOutputStream out = outputStream;
        if (out == null) return;
        try (io.github.dd1android.launcher.xconnector.XStreamLock lock = out.lock()) {
    ...
    public void freeResources() {
        try (XLock lock = xServer.lockAll()) {
            ...
            while (!eventListeners.isEmpty()) {
                Map.Entry<Window, EventListener> entry = eventListeners.entrySet().iterator().next();
