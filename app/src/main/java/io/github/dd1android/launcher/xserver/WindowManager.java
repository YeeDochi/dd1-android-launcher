...
import io.github.dd1android.launcher.xserver.errors.BadValue;
import io.github.dd1android.launcher.xserver.errors.XRequestError;
import io.github.dd1android.launcher.xserver.events.ConfigureNotify;
import io.github.dd1android.launcher.xserver.events.ConfigureRequest;
import io.github.dd1android.launcher.xserver.events.DestroyNotify;
import io.github.dd1android.launcher.xserver.events.Event;
import io.github.dd1android.launcher.xserver.events.Expose;
import io.github.dd1android.launcher.xserver.events.MapNotify;
import io.github.dd1android.launcher.xserver.events.MapRequest;
import io.github.dd1android.launcher.xserver.events.ResizeRequest;
import io.github.dd1android.launcher.xserver.events.UnmapNotify;
...
public class WindowManager extends XResourceManager {
            ...
            public void run() {
                ...
                try {
                    for (int i = 0; i < 2; i++) {
                        ...
                        android.util.Log.i("DD1/XServer", "ConfigureNotify KICK #" + (i + 1)
                    ...
                    android.util.Log.w("DD1/XServer", "configure-kick failed (non-fatal)", e);
                }
            }
        }, "rd-configure-kick");
        t.setDaemon(true);
    ...
    public void mapWindow(Window window) {
        android.util.Log.i("DD1/XServer", "MAPWIN win=0x" + Integer.toHexString(window.id)
        ...
        if (!window.attributes.isMapped()) {
            ...
            if (!parent.hasEventListenerFor(Event.SUBSTRUCTURE_REDIRECT) || window.attributes.isOverrideRedirect()) {
                ...
                window.sendEvent(Event.VISIBILITY_CHANGE, new io.github.dd1android.launcher.xserver.events.VisibilityNotify(window));
    ...
    public void unmapWindow(Window window) {
        android.util.Log.i("DD1/XServer", "UNMAPWIN win=0x" + Integer.toHexString(window.id)
    ...
    public void setFocus(Window focusedWindow, FocusRevertTo focusRevertTo) {
        ...
        if (old != null && old != rootWindow)
            old.sendEvent(Event.FOCUS_CHANGE, new io.github.dd1android.launcher.xserver.events.FocusOut(old));
        if (focusedWindow != null && focusedWindow != rootWindow)
            focusedWindow.sendEvent(Event.FOCUS_CHANGE, new io.github.dd1android.launcher.xserver.events.FocusIn(focusedWindow));
    }
    ...
    public void configureWindow(Window window, Bitmask valueMask, XInputStream inputStream) throws XRequestError {
        ...
        if (!parent.hasEventListenerFor(Event.SUBSTRUCTURE_REDIRECT) || overrideRedirect) {
            ...
            if (unchanged) {
                android.util.Log.i("DD1/XServer", "ConfigureWindow no-op (same geometry) win=0x"
    ...
    public void reparentWindow(Window window, Window newParent) {
        ...
        android.util.Log.i("DD1/XServer", "reparent win=0x" + Integer.toHexString(window.id)
        ...
        window.sendEvent(Event.STRUCTURE_NOTIFY,
                new io.github.dd1android.launcher.xserver.events.ReparentNotify(window, window, newParent));
        newParent.sendEvent(Event.SUBSTRUCTURE_NOTIFY,
                new io.github.dd1android.launcher.xserver.events.ReparentNotify(newParent, window, newParent));
        if (oldParent != null && oldParent != newParent)
            oldParent.sendEvent(Event.SUBSTRUCTURE_NOTIFY,
                    new io.github.dd1android.launcher.xserver.events.ReparentNotify(oldParent, window, newParent));
        if (wasMapped) {
            updateWmState(window, WM_STATE_NORMAL);
            window.sendEvent(Event.STRUCTURE_NOTIFY, new MapNotify(window, window));
    ...
    private void updateWmState(Window window, int state) {
        ...
        if (property != null) {
            ...
            Log.i("DD1/XServer", "WM_STATE win=0x" + Integer.toHexString(window.id)
