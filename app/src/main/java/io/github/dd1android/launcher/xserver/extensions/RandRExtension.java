...
public class RandRExtension extends Extension {
            ...
            public void run() {
                for (int i = 0; i < 3; i++) {
                    ...
                    if (target == null || target.getOutputStream() == null) {
                        android.util.Log.w("DD1/XServer", "RANDR notify #" + (i + 1) + ": no live client, skipping");
                        continue;
                    }
                    try {
                        android.util.Log.i("DD1/XServer", "RANDR -> RRScreenChangeNotify " + w + "x" + h + " (#" + (i + 1) + ")");
                        target.sendEvent(new io.github.dd1android.launcher.xserver.events.RRScreenChangeNotify(
                                FIRST_EVENT, rootId, rootId, w, h, mw, mh, TIMESTAMP));
                    } catch (Exception e) {
                        android.util.Log.w("DD1/XServer", "RRScreenChange send err: " + e);
                    }
                }
            }
        }, "rd-randr-notify");
    ...
    public void handleRequest(XClient client, XInputStream inputStream, XOutputStream outputStream) throws IOException, XRequestError {
        ...
        android.util.Log.i("DD1/XServer", "RANDR req minor=" + opcode
        ...
        switch (opcode) {
            ...
            case ClientOpcodes.LIST_OUTPUT_PROPERTIES:
                ...
                break;
            default:
                android.util.Log.w("DD1/XServer", "RANDR: unsupported minor opcode " + opcode);
                client.skipRequest();
